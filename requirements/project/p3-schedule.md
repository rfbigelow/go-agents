...
|-----------|-------------|------------|
| M1: Basic Conversation | Agent can perform simple LLM completions with streaming. Client wraps Anthropic SDK. Conversation state is managed by the library. Tracing spans and structured logs are emitted for the agent run and LLM calls. | — |
...
- **Early iterations (M1–M2):** Focus on core abstractions, API design, and
  the fundamental conversation loop. Observability (tracing and logging)
