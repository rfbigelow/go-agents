...
|-----------|-------------|------------|
| M1: Basic Conversation | Agent can perform simple LLM completions with streaming. Client wraps Anthropic SDK. Conversation state is managed by the library. | — |
| M2: Tool Use | Agent supports tool registration and the full conversation loop (tool dispatch, result handling, loop termination). | M1 |
...
- **Early iterations (M1–M2):** Focus on core abstractions, API design, and
  the fundamental conversation loop. These establish the foundation that all
  later capabilities build on.
