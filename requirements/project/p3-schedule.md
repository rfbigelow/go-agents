...
| M9: Conversation Resumption | Complete | Implements S2.15 — `NewAgentWithHistory` constructs an Agent from a prior message history, validating it against the five resumption invariants (with a typed error identifying the violated rule) and round-tripping with `messages` (S2.6), thinking blocks and signatures preserved verbatim. Two invariants were tightened during implementation (see `changes/2026-07-04-resumption-invariant-clarifications.md`). Closed issue #4. | M1 |
| M10: Context Compaction | Complete | Implements opt-in conversation compaction (S2.18–S2.21): the `CompactionStrategy` extension point (S3.6) with commit/transient semantics, manual/proactive/reactive triggers (S2.19), token usage reporting via `Agent.Usage` (S2.20), library-provided hybrid-summarization and sliding-window strategies (S2.21), and archival of the replaced prefix for lossless resume through the M9 round-trip. Two spec points were clarified during implementation (see `changes/2026-07-18-compaction-implementation-clarifications.md`). | M8, M9 |
...
- **Later iterations (M9–M10, shipped):** Support long-running agents.
  Conversation resumption (M9) closed the S2.15 gap and unblocked context
  compaction (M10), whose archival path re-injects the replaced prefix
  through the resumption round-trip.
